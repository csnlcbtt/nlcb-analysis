...
// interface for processed Cashpot data
export interface CashpotData extends GameData {
  allNumbers?: string[];
}
...
export function SearchTabContent() {
  ...
  const [searchResults, setSearchResults] = useState<CashpotData[]>([]);
  ...
  useEffect(() => {
    ...
    // Debug CSV data
    console.log("Cashpot Master data sample:", masterData?.slice(0, 5));
    console.log("Cashpot DOW data sample:", dowData?.slice(0, 5));
    console.log("Cashpot DOM data sample:", domData?.slice(0, 5));
    console.log("Cashpot Holidays data sample:", holidaysData?.slice(0, 5));
  ...
  function processData(data: CsvData[]): CashpotData[] {
    return data.map((item) => {
      const drawData = item as unknown as CashpotData;
      
      // Process the numbers (Cashpot has multiple numbers)
      if (drawData.Nums && typeof drawData.Nums === 'string') {
        // Convert comma-separated numbers to array
        drawData.allNumbers = drawData.Nums.split(',').map(num => num.trim().padStart(2, '0'));
      }
  ...
  const applyDateRangeFilter = (data: CashpotData[]): CashpotData[] => {
  ...
  const sortResults = (
    ...
    return [...data].sort((a, b) => {
      if (sortField === 'date') {
        ...
        // For Cashpot, we'll sort by the first number in the set
        const numA = a.allNumbers && a.allNumbers.length > 0 ? parseInt(a.allNumbers[0]) : 0;
        const numB = b.allNumbers && b.allNumbers.length > 0 ? parseInt(b.allNumbers[0]) : 0;
  ...
  return (
    <Card className="p-6">
      <h3 className="text-xl font-semibold mb-4">Search Cashpot Results</h3>
          ...
          {searchType === 'drawNumber' && (
            <>
              ...
              <Input
                ...
                value={drawNumber}
                onChange={(e) => setDrawNumber(e.target.value.trim())}
                    ...
                    displayedResults.data.map((result, idx) => (
                      <TableRow key={idx} className="hover:bg-muted/50">
                        ...
                        <TableCell className="flex gap-1">
                          {result.allNumbers && result.allNumbers.map((num, index) => (
      ...
      {searchResults.length > 0 && (
        <Button 
          ...
          onClick={() => {
            // Create CSV content
            const headers = ["Draw Number", "Date", "Day", "Numbers"];
            const rows = searchResults.map(item => [
              String(item.DrawNo),
              item.DrawDates.split(' ')[1],
              item.day || '',
              item.allNumbers ? item.allNumbers.join(',') : item.Nums
            ]);
            ...
            link.setAttribute('download', `cashpot-search-results-${new Date().toISOString().split('T')[0]}.csv`);
            link.click();
          }}
          className="ml-2"
        >
