...
// interface for processed Pick4 data
export interface Pick4Data extends GameData {
  FirstDigit?: string;
  SecondDigit?: string;
  ThirdDigit?: string;
  FourthDigit?: string;
...
export function SearchTabContent() {
  ...
  const [searchResults, setSearchResults] = useState<Pick4Data[]>([]);
  ...
  useEffect(() => {
    ...
    // Debug CSV data
    console.log("Pick4 Master data sample:", masterData?.slice(0, 5));
    console.log("Pick4 DOW data sample:", dowData?.slice(0, 5));
    console.log("Pick4 DOM data sample:", domData?.slice(0, 5));
    console.log("Pick4 Holidays data sample:", holidaysData?.slice(0, 5));
  ...
  function processData(data: CsvData[]): Pick4Data[] {
    return data.map((item) => {
      const drawData = item as unknown as Pick4Data;
      
      // Ensure Nums is padded to 4 digits for Pick4
      if (drawData.Nums && typeof drawData.Nums === 'string') {
        drawData.Nums = drawData.Nums.padStart(4, '0');
        
        // Split the number into digits for Pick4
        if (drawData.Nums.length === 4) {
          drawData.FirstDigit = drawData.Nums[0];
          drawData.SecondDigit = drawData.Nums[1];
          drawData.ThirdDigit = drawData.Nums[2];
          drawData.FourthDigit = drawData.Nums[3];
        }
      }
  ...
  const applyDateRangeFilter = (data: Pick4Data[]): Pick4Data[] => {
  ...
  return (
    <Card className="p-6">
      <div className="mb-6">
        <h3 className="text-lg font-semibold mb-2">Search Pick 4 Numbers</h3>
        <p className="text-sm text-muted-foreground mb-4">
          Search for Pick 4 numbers by draw number, date, weekday, holiday, or day of the month.
        </p>
                            ...
                            <TooltipProvider>
                              <Tooltip>
                                <TooltipTrigger asChild>
                                  <Badge className="w-[50px] flex justify-center">
                                    {result.Nums}
                                  </Badge>
                                </TooltipTrigger>
                                <TooltipContent>
                                  <p>Pick4 Number: {result.Nums}</p>
                                </TooltipContent>
                              </Tooltip>
                            </TooltipProvider>
                          </TableCell>
        ...
        {searchResults.length > 0 && (
          <Button 
            ...
            onClick={() => {
              ...
              link.setAttribute('download', `pick4-search-results-${new Date().toISOString().split('T')[0]}.csv`);
              link.click();
            }}
            className="ml-2"
          >
