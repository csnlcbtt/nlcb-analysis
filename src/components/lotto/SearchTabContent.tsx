...
// Interface for processed Lotto data
export interface LottoData extends GameData {
  AllNos: string;
  PB: string;
  MU: string;
  Wins: string;
...
export function SearchTabContent() {
  ...
  const [searchResults, setSearchResults] = useState<LottoData[]>([]);
  ...
  const processedData = useMemo(() => {
    ...
    return masterData.map((item) => {
      const drawData = item as unknown as LottoData;
    ...
    try {
      switch (searchType) {
        case 'drawNumber':
          ...
          results = processedData.filter(item => 
            item.DrawNo.toString() === drawNumber
          );
          break;
        case 'drawDate':
          if (!drawDate) {
        ...
        case 'holiday':
          ...
          if (holidaysData) {
            ...
            results = processedData.filter(item => 
              holidayDraws.includes(item.DrawNo.toString())
            );
          }
          break;
        case 'dayOfMonth':
      ...
      results.sort((a, b) => {
        if (sortBy === 'date') {
          ...
          // For Lotto, sorting by number would typically be by DrawNo
          const aNum = a.DrawNo;
          const bNum = b.DrawNo;
  ...
  const exportToCSV = () => {
    ...
    // Create CSV content
    const headers = ['DrawNo', 'DrawDates', 'AllNos', 'PB', 'MU', 'Wins', 'Draws', 'Nfpd'];
    const csvContent = [
      headers.join(','),
    ...
    link.setAttribute('download', `lotto_search_${format(new Date(), 'yyyy-MM-dd')}.csv`);
  ...
  return (
    <Card className="p-6">
      <div className="mb-6">
        <h3 className="text-lg font-semibold mb-2">Search Lotto Numbers</h3>
        <p className="text-sm text-muted-foreground mb-4">
          Search for Lotto numbers by draw number, date, weekday, holiday, or day of the month.
        </p>
                ...
                <Table>
                  ...
                  <TableBody>
                    {paginatedResults.map((result, index) => (
                      <TableRow key={index}>
                        <TableCell className="font-medium">{result.DrawNo}</TableCell>
      ...
      {!searchPerformed && !isSearching && (
        <div className="text-center py-8">
          <p className="text-muted-foreground">Select search criteria and click Search to find Lotto numbers.</p>
        </div>
      )}
    </Card>
  );
