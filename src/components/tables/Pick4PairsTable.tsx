...
export function Pick4PairsTable() {
  const { data, loading, error } = useCsvData('/csv/p4pairs.csv');
  ...
  if (error) {
    return <div className="text-red-500 p-4">Failed to load pairs: {error.message}</div>;
  }

  const processedData = data
    .map(row => ({
      pair: String(row.Combi).padStart(2, '0'), // Format as 2-digit string
