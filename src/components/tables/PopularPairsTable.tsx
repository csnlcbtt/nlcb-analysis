...
export function PopularPairsTable() {
  const { data, loading, error } = useCsvData('/csv/pwcpairs.csv');
  ...
  if (error) {
    return <div className="text-red-500 p-4">Failed to load popular pairs: {error.message}</div>;
  }

  // Map and sort all data
  const processedData = data
    .map(row => ({
      pair: row.Combinations,
