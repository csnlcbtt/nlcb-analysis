...
export function WinForLifeTripletsTable() {
  const { data, loading, error } = useCsvData('/csv/wlctrips.csv');
  const [pageSize, setPageSize] = useState(10);
  const [currentPage, setCurrentPage] = useState(1);

  if (loading) return <Skeleton className="h-64 w-full" />;
  if (error) return <div className="text-red-500 p-4">Failed to load triplets: {error.message}</div>;

  const processedData = data
    .map(row => ({
      triplet: row["Combinations"] || row["Combi"],
