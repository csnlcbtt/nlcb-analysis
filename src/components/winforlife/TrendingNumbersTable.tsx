...
export function WinForLifeTrendingNumbersTable() {
  const { data, loading, error } = useCsvData('/csv/wlnctr.csv');
  const [pageSize, setPageSize] = useState(10);
  const [currentPage, setCurrentPage] = useState(1);

  if (loading) return <Skeleton className="h-64 w-full" />;
  if (error) return <div className="text-red-500 p-4">Failed to load trending numbers: {error.message}</div>;

  const processedData = data
    .map(row => ({
      no: row.No,
      all: Number(row.PrvAllDr),
      last6: Number(row.Prv6Dr),
      last12: Number(row.Prv12Dr),
      last18: Number(row.Prv18Dr),
      last24: Number(row.Prv24Dr),
      last30: Number(row.Prv30Dr)
    }))
    ...
    <div>
      ...
      <DataTable
        ...
        columns={[
          { key: 'no', header: 'No' },
          { key: 'all', header: 'All' },
          { key: 'last6', header: 'Last 6' },
          { key: 'last12', header: 'Last 12' },
          { key: 'last18', header: 'Last 18' },
          { key: 'last24', header: 'Last 24' },
          { key: 'last30', header: 'Last 30' }
        ]}
      />
      {totalPages > 1 && (
