...
interface Pick4ChartProps {
  chartNumber: number;
  width: number; // Width as percentage
  active: boolean;
}

export function Pick4Chart({ chartNumber, width, active }: Pick4ChartProps) {
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  
  const chartId = chartNumber.toString().padStart(2, '0');
  const imgPath = `/img/P4${chartId}.png`;
  
  const chartData = useCsvData('/csv/p4wkass.csv');
  ...
  if (error) {
    ...
    return (
      <div className={cn("relative border border-border rounded-md overflow-hidden w-full", maxWidthClass)}>
